...
template <typename Iterator, typename Skipper> struct grammar : qi::grammar<Iterator, value_t(), Skipper>
{
	grammar()
		: grammar::base_type{value_rule}
	{
		qi::lit_type			lit;
		ascii::char_type		chr;
		...
		value_rule = qi::bool_ | lexeme[!('+' | (-lit('-') >> '0' >> digit)) >> qi::int_ >> !chr(".eE")] |
					 lexeme[!('+' | (-lit('-') >> '0' >> digit)) >> qi::double_] | array_rule | struct_rule |
					 string_rule | quote_rule | bytestring_rule;
...
template <typename Iterator, typename Skipper> struct uri_grammar : qi::grammar<Iterator, uri_t(), Skipper>
{
	uri_grammar()
		: uri_grammar::base_type{uri_rule}
	{
		ascii::char_type chr;
		qi::lexeme_type  lexeme;
...
template <typename Iterator, typename Skipper> struct target_grammar : qi::grammar<Iterator, target_t(), Skipper>
{
	target_grammar()
		: target_grammar::base_type{target_rule}
	{
		ascii::char_type chr;
		qi::lexeme_type  lexeme;
